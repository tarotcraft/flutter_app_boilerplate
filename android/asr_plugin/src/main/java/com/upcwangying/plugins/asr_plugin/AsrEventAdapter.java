 ...
 */
public class AsrEventAdapter implements EventListener {

    private OnAsrListener listener;

    private static final String TAG = "AsrEventAdapter";

    public AsrEventAdapter(OnAsrListener listener) {
    ...
    public void onEvent(String name, String params, byte[] data, int offset, int length) {
        ...
        if (name.equals(SpeechConstant.CALLBACK_EVENT_ASR_LOADED)) {
            ...
            AsrResult AsrResult = com.upcwangying.plugins.asr_plugin.AsrResult.parseJson(params);
            // 识别结果
            String[] results = AsrResult.getResultsRecognition();
            if (AsrResult.isFinalResult()) {
                // 最终识别结果，长语音每一句话会回调一次
                listener.onAsrFinalResult(results, AsrResult);
            } else if (AsrResult.isPartialResult()) {
                // 临时识别结果
                listener.onAsrPartialResult(results, AsrResult);
            } else if (AsrResult.isNluResult()) {
                // 语义理解结果
            ...
            // 识别结束
            AsrResult AsrResult = com.upcwangying.plugins.asr_plugin.AsrResult.parseJson(params);
            if (AsrResult.hasError()) {
                int errorCode = AsrResult.getError();
                int subErrorCode = AsrResult.getSubError();
                Log.e(TAG, "asr error:" + params);
                listener.onAsrFinishError(errorCode, subErrorCode, AsrResult.getDesc(), AsrResult);
            } else {
                listener.onAsrFinish(AsrResult);
            }
